//! Profile V002 block compression on a real snapshot.
//!
//! Opens a V001 snapshot, re-encodes every block in the current V002 format
//! (common-prefix dedup + zstd), and reports the on-disk size reduction. The
//! V001 baseline is the table's own recorded block-data size; the V002 figure
...
//! ```
//!
//! Round-trip correctness of the V002 codec is covered by the unit and
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut v002_total = 0u64; // re-encoded framed V002 block bytes
    for bn in 0..block_num {
        let block = r.load_block(bn)?;

        let mut buf = Vec::new();
        block.encode(&mut buf)?;
        v002_total += buf.len() as u64;
    ...
    println!(
        "V002 blocks on-disk: {v002_total} B ({:.1} MiB)",
        mib(v002_total)
    );
    println!(
        "V001 -> V002       : {:.2}x  ({:.1}% smaller; prefix dedup + zstd)",
        v001_total as f64 / v002_total as f64,
        100.0 * (1.0 - v002_total as f64 / v001_total as f64),
    );
    Ok(())
}
