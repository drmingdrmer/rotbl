...
fn test_dump<S: Storage>(ctx: TestContext<S>) -> anyhow::Result<()> {
    ...
    let want = vec![
        ...
        r#"    stat: 4 keys in 2 blocks: data(218 B), index(190 B), avg block size(109 B)"#,
        r#"    access_stat: AccessStat { read_key: 0, read_block: 0, read_block_from_cache: 0, read_block_from_disk: 0 }"#,
        r#"BlockIndex: n: 2"#,
        r#"    index: { block_num: 0000, position: 36+115, key_range: ["a", "c"] }"#,
        r#"    index: { block_num: 0001, position: 151+103, key_range: ["d", "d"] }"#,
